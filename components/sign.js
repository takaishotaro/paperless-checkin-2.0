...
//api--------------------------------------------
  ...
  return(
    <div>
      ...
      />
      <Button className="m-1" onClick={clear}>{props.lang==="JP" ? "書き直し":"clear"}</Button>
      <Button className="m-1" onClick={save} color="primary" disabled={!isValidOperation()}>{props.lang==="JP" ? "署名完了":"done signing"}</Button>
      { loading && (
        <div className="mb-3">
